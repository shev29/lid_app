...
const defaultOptions = {
  ...
  standardFontDataUrl: {
    value: "js/app/pdf/standard_fonts/",
    kind: OptionKind.API
  },
  useSystemFonts: {
    value: undefined,
...
};
{
  ...
  defaultOptions.sandboxBundleSrc = {
    value: "js/app/pdf/build/pdf.sandbox.js",
    kind: OptionKind.VIEWER
  };
...
const PDFViewerApplication = {
  ...
  async run(config) {
    ...
    // token = "token" in params ? 'http://localhost/hr_new/index.php/render/?token='+params.token : AppOptions.get("defaultUrl");
    token = params.get("token") ? '/renderPdf/?token='+params.get("token") : AppOptions.get("defaultUrl");
    // console.log(token);
  ...
  load(pdfDocument) {
    ...
    firstPagePromise.then(pdfPage => {
      ...
      Promise.all([animationStarted, storedPromise, pageLayoutPromise, pageModePromise, openActionPromise]).then(async ([timeStamp, stored, pageLayout, pageMode, openAction]) => {
        ...
        // const zoom = "page-actual";
        const zoom = "page-width";
        ...
        if (stored?.page && viewOnLoad !== ViewOnLoad.INITIAL) {
          ...
          hash = `page=1&zoom=${zoom || stored.zoom}`;
          // hash = `page=1&zoom=auto,` + `${stored.scrollLeft},${stored.scrollTop}`;
          // hash = `page=1&zoom=100`;
...
}
{
  ...
  var validateFileURL = function (token) {
    if (!token) {
      return;
    }
    try {
      ...
      const fileOrigin = new URL(token, window.location.href).origin;
